...
function writeToLogFile (message) {
  try {
    ...
    const logFileName = `ollama-${dateStr}.log`
...
async function callOllamaAPI (options) {
  const {
    model,
    prompt,
    ollamaUrl,
  ...
  const chatUrl = ollamaUrl.startsWith('http://') ||
    ...
    ? `${ollamaUrl}/api/chat`
    : `http://${ollamaUrl}/api/chat`

  const generateUrl = ollamaUrl.startsWith('http://') ||
    ollamaUrl.startsWith('https://')
    ? `${ollamaUrl}/api/generate`
    : `http://${ollamaUrl}/api/generate`

  const passLabel = passName ? ` [${passName}]` : ''
  const requestLog =
    `🤖 Ollama API Request${passLabel}:\n   Model: ${model}\n   ` +
  ...
  try {
    ...
    if (numCtx !== null && numCtx !== undefined) {
      chatRequestBody.options.num_ctx = numCtx
      writeToLogFile(`   Context window (num_ctx): ${numCtx} tokens`)
    }

    if (format) {
      chatRequestBody.format = format
      const formatStr = JSON.stringify(format).substring(0, 100)
      writeToLogFile(
        `   Using structured output format: ${formatStr}...`
      )
    }
    ...
    try {
      url = chatUrl
      writeToLogFile(`   Using Chat API: ${url}`)
      response = await axios.post(url, chatRequestBody, {
        headers: {
      ...
      writeToLogFile(
        `   Chat API failed, using Generate API: ${chatError.message}`
      )
      url = generateUrl
      ...
      if (numCtx !== null && numCtx !== undefined) {
        generateRequestBody.options.num_ctx = numCtx
      }
      response = await axios.post(url, generateRequestBody, {
        headers: {
          'Content-Type': 'application/json'
        },
        timeout: 0
    ...
    writeToLogFile(`   API URL used: ${url}`)
